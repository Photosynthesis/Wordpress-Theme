...
<?php if ( $has_downloads ) : ?>
	...
	<table class="table table-sm table-striped table-bordered woocommerce-MyAccount-downloads shop_table shop_table_responsive">
		<thead>
			<tr>
										...
										<?php
											...
											if ( $actions = apply_filters( 'woocommerce_account_download_actions', $actions, $download ) ) {
												foreach ( $actions as $key => $action ) {
													echo '<a href="' . esc_url( $action['url'] ) . '" class="btn btn-sm btn-secondary btn-block button woocommerce-Button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
												}
											}
										?>
										<?php break;
...
<?php else : ?>
	<div class="woocommerce-Message woocommerce-Message--info woocommerce-info alert alert-info">
