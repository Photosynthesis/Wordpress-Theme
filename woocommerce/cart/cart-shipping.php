...
<tr class="shipping">
	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( 1 < count( $available_methods ) ) : ?>
			<ul id="shipping_method" class='list-unstyled mb-0'>
				<?php foreach ( $available_methods as $method ) : ?>
					<li class='form-check mb-1'>
						<?php
							printf( '<label class="form-check-label"><input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="form-check-input shipping_method" %4$s />
								%5$s</label>',
