...
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS public
ON public.item_id=items.id AND public.field_id=218
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS country
ON country.item_id=items.id AND country.field_id=424
WHERE country.meta_value IS NOT NULL AND public.meta_value='Yes'
GROUP BY country.meta_value
ORDER BY country.meta_value ASC) AS countries
LEFT JOIN (SELECT state.meta_value AS state, COUNT(state.meta_value) AS state_count, country.meta_value AS country
FROM " . $wpdb->prefix . "frm_items AS items
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS public
ON public.item_id=items.id AND public.field_id=218
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS country
ON country.item_id=items.id AND country.field_id=424
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS state
ON state.item_id=items.id AND state.field_id=815
WHERE state.meta_value IS NOT NULL AND public.meta_value='Yes'
AND country.meta_value='United States'
GROUP BY state.meta_value
ORDER BY state.meta_value ASC) AS states
ON states.country=countries.country
LEFT JOIN (SELECT prov.meta_value AS prov, COUNT(prov.meta_value) AS prov_count, country.meta_value AS country
FROM " . $wpdb->prefix . "frm_items AS items
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS public
ON public.item_id=items.id AND public.field_id=218
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS country
ON country.item_id=items.id AND country.field_id=424
LEFT JOIN " . $wpdb->prefix . "frm_item_metas AS prov
ON prov.item_id=items.id AND prov.field_id=428
WHERE prov.meta_value IS NOT NULL AND public.meta_value='Yes'
AND country.meta_value!='United States'
GROUP BY prov.meta_value
ORDER BY prov.meta_value ASC) AS provs
