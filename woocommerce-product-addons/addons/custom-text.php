...
$price_raw         = apply_filters( 'woocommerce_product_addons_option_price', '1' == $adjust_price && $price ? $price : '', $addon );
...
<p class="form-row form-row-wide wc-pao-addon-wrap wc-pao-addon-<?php echo sanitize_title( $field_name ); ?>">
	<?php if ( 'email' === $restrictions_type ) { ?>
		<input type="email" class="form-control input-text wc-pao-addon-field wc-pao-addon-custom-text" data-raw-price="<?php echo esc_attr( $price_raw ); ?>" data-price="<?php echo esc_attr( $price_display ); ?>" name="<?php echo esc_attr( $addon_key ); ?>" data-price-type="<?php echo esc_attr( $price_type ); ?>" value="" <?php if ( WC_Product_Addons_Helper::is_addon_required( $addon ) ) { echo 'required'; } ?> />
	<?php } else { ?>
