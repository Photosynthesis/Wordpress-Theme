<?php
/** General Functions for Maniuplating the Directory's Formidable Database 
 * 
 * @category FIC
 * @package FIC_DIR_DB
...
class FIC_DIR_DB
{
    ...
    public static function update_or_insert_item_meta($field_id, $item_id, $value) {
        $meta_id = FIC_DIR_DB::get_item_meta_id($field_id, $item_id);
        if ($meta_id !== false) {
            FIC_DIR_DB::update_item_meta($meta_id, $value);
        } else {
            FIC_DIR_DB::insert_item_meta($field_id, $item_id, $value);
        }
    }
