...
/** Show links to filter the directory search results page by:
 *
 * * Is Established?
 * * Is Forming?
 * * Visitors Welcome?
 ...
 *
 * The links are generated by modifying the GET parameters of the current URL.
 *
 * @return string HTML containg the links
 */
function directory_show_search_filters() {
    ...
    // Check if a filter is used & remove it from the GET parameters
    foreach ($filters as &$filter) {
        if (array_key_exists($filter['param'], $_GET) &&
            strpos($_GET[$filter['param']], $filter['value']) !== false) {
                $filter['used'] = true;
                unset($_GET[$filter['param']]);
        } else {
            $filter['used'] = false;
        }
    }
    ...
    foreach ($filters as &$filter) {
        if ($filter['used'] === true) {
            $links[] = "<b>{$filter['text']}</b> ";
        } else {
            $new_get = $_GET;
            $new_get[$filter['param']] = $filter['value'];
            $url = http_build_query($new_get);
            $links[] = "<a href='.?{$url}'>{$filter['text']}</a> ";
        }
    }

    $link_html = join(' | ', $links);

    return "<div style='float:right;'>Filters: {$link_html}</div><div style='clear:both;'></div>";
}
