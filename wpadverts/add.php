...
<form action="" method="post">
    <fieldset>
        ...
        <div class="form-group <?php echo esc_attr( str_replace("_", "-", $field["type"] ) . " adverts-field-name-" . $field["name"] ) ?> <?php if(adverts_field_has_errors($field)): ?>adverts-field-error<?php endif; ?>">

            <?php if($field["type"] == "adverts_field_header"): ?>
            <div class="adverts-field-header">
                <span class="h5 adverts-field-header-title"><?php echo esc_html($field["label"]) ?></span>
        ...
        <div>
            <input type="submit" name="submit" value="<?php _e("Preview", "adverts") ?>" class="btn btn-primary" />
        </div>
